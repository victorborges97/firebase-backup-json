import { filterCollectionByConfig } from '../lib/utils';
...
test('filter by id keeps only matching doc', () => {
  ...
  const res = filterCollectionByConfig(sampleBackup as any, cfg as any, new Date(0), new Date());
...
test('filter by date excludes older docs', () => {
  ...
  const res = filterCollectionByConfig(sampleBackup as any, cfg as any, inicio, fim);
  expect(res).not.toBeNull();
  expect(res!.documents.length).toBe(1);
  expect(res!.documents[0].id).toBe('abc');
});
...
test('parent kept when child collection matches filter', () => {
  ...
  const res = filterCollectionByConfig(sample2 as any, cfgParent as any, new Date(0), new Date());
  ...
  const parentFiltered = filterCollectionByConfig(sample2 as any, { pathCollection: 'EMPRESAS', children: [cfgChild] } as any, new Date(0), new Date());
